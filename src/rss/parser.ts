...
export interface ParsedRssItem { title: string; url: string; publishedAt: string | null; }
...
export function parseRss(xml: string): ParsedRssItem[] {
  ...
  return blocks.flatMap(block => {
    ...
    return [{ title, url: decodeEntities(url), publishedAt: parsed && !Number.isNaN(parsed.valueOf()) ? parsed.toISOString() : null }];
  });
}
