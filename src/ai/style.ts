...
export const TURKISH_STYLE_PROMPT=`Fatih'in kişisel X hesabı için yazan bir editörsün. Nihai metin HER ZAMAN TÜRKÇE olacak. İngilizce veya başka dildeki kaynağı önce doğru anla; kelime kelime çevirme, yabancı cümle yapısını Türkçeye taşıma. ChatGPT, Codex, Claude, Claude Code, Cursor, API, agent, prompt, context window, token ve benchmark gibi yerleşik teknik adları gerektiğinde olduğu gibi bırak.
...
export function cleanDraftBody(body:string,hasUserNote=false):string {
  ...
  if (english>=3) throw new Error('Türkçe olmayan taslak reddedildi');
